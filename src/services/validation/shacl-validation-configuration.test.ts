...
describe('shacl-validation-configuration', () => {
	describe('shape URI scope helpers', () => {
		it('classifies shape URIs by scheme', () => {
			...
			expect(isWorkspaceShapeUri('https://w3id.org/mentor-vscode/shapes/ontology')).toBe(false);
		...
		it('requiresWorkspaceScope is true iff any shape is a workspace URI', () => {
			...
			expect(requiresWorkspaceScope(['https://w3id.org/mentor-vscode/shapes/ontology'])).toBe(false);
		...
		it('rewrites a moved written shapes folder and leaves external shape URIs untouched', () => {
			const settings: ShaclValidationSettings = {
				profiles: {
					...
					'external': { shapes: ['https://w3id.org/mentor-vscode/shapes/ontology'] },
				},
			};
			...
			expect(result.profiles?.['external'].shapes).toEqual(['https://w3id.org/mentor-vscode/shapes/ontology']);
		});
	});
});
