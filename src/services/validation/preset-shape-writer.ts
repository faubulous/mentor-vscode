 ...
 *
 * The file is written to `<shapesFolder>/<presetId>.shape.ttl` (see the
 ...
 *
 * @param presetId The built-in preset id (e.g. `'basic-ontology'`).
...
export async function writePresetShapes(presetId: string): Promise<WrittenPresetShapes> {
	...
	const target = await resolveTargetUri(root, folder, presetId, source);
 ...
 * `<presetId>.shape.ttl` when free or already holding identical content (reused),
 * otherwise the first free `<presetId>-N.shape.ttl`.
 */
async function resolveTargetUri(
	...
	for (let index = 1; ; index++) {
		const name = index === 1 ? `${presetId}.shape.ttl` : `${presetId}-${index}.shape.ttl`;
