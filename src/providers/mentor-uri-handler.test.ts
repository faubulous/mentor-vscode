...
import { MentorUriHandler } from '@src/providers/mentor-uri-handler';

describe('MentorUriHandler', () => {
	let handler: MentorUriHandler;
	...
	it('ignores URIs with wrong authority', async () => {
		...
		expect(mockRouter.open).not.toHaveBeenCalled();
	});

	it('ignores URIs with unknown path', async () => {
	...
	describe('/settings', () => {
		it('opens the settings root when no section is given', async () => {
			const uri = vscode.Uri.parse('vscode://test.extension/settings');
			await handler.handleUri(uri);
			expect(mockRouter.open).toHaveBeenCalledWith(
				{ kind: 'settings', section: undefined },
				vscode.ViewColumn.Active
			);
		});

		it('opens a valid section', async () => {
			const uri = vscode.Uri.parse('vscode://test.extension/settings?section=query.connections');
			await handler.handleUri(uri);
			expect(mockRouter.open).toHaveBeenCalledWith(
		...
		it('falls back to the root for an unknown section', async () => {
			const uri = vscode.Uri.parse('vscode://test.extension/settings?section=bogus');
			await handler.handleUri(uri);
			expect(mockRouter.open).toHaveBeenCalledWith(
				{ kind: 'settings', section: undefined },
				vscode.ViewColumn.Active
