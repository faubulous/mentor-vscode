...
export const toggleSparqlConnectionInference = {
	...
	handler: async (connection?: SparqlConnection) => {
		...
		const targetConnection = connectionRegistry.getConnection(connectionId);
		...
		if (!storeConfigService.supportsInference(targetConnection)) {
			vscode.window.showErrorMessage(`This connection does not support inference toggling.`);
			return;
		}

		const newValue = await connectionRegistry.toggleInferenceEnabled(connectionId);
