 ...
 * Positions are 1-based (Chevrotain convention) and place the token at line 1, col `startColumn`.
 */
function makeToken(name: string, image: string, startColumn = 1) {
    return { tokenType: { name }, image, startLine: 1, startColumn, endLine: 1, endColumn: startColumn + image.length - 1 };
}
...
describe('SparqlCompletionItemProvider', () => {
    ...
    describe('isGraphDefinitionContext', () => {
        it('returns true when current token starts with < and previous is GRAPH', () => {
            ...
            expect(provider.isGraphDefinitionContext(tokens, 1)).toBe(true);
        });
        ...
        it('returns true when previous token is < and token before that is GRAPH', () => {
            ...
            expect(provider.isGraphDefinitionContext(tokens, 2)).toBe(true);
        });
        ...
        it('returns false when previous token is not GRAPH, FROM, or NAMED', () => {
            ...
            expect(provider.isGraphDefinitionContext(tokens, 1)).toBe(false);
        });
        ...
        it('returns false when previous token is < but the one before is not GRAPH/FROM/NAMED', () => {
            ...
            expect(provider.isGraphDefinitionContext(tokens, 2)).toBe(false);
        });
    ...
    describe('getGraphIriCompletionItems', () => {
        ...
        it('filters graphs by the already-typed prefix', async () => {
            ...
            const tokens = [makeToken(RdfToken.IRIREF.name, '<http://example')] as any;
            const items = await p.getGraphIriCompletionItems(makeDoc('file:///test.sparql', '<http://example') as any, tokens, 0);
        ...
        it('strips the trailing > from a closed token image', async () => {
            ...
            const tokens = [makeToken(RdfToken.IRIREF.name, '<htt>')] as any;
            const items = await p.getGraphIriCompletionItems(makeDoc() as any, tokens, 0);
        ...
        it('returns empty array when no graphs match', async () => {
            const p = makeProviderWithGraphs(graphs);
            const tokens = [makeToken(RdfToken.IRIREF.name, '<https://nomatch')] as any;
            const items = await p.getGraphIriCompletionItems(makeDoc('file:///test.sparql', '<https://nomatch') as any, tokens, 0);
        ...
        it('shows the full IRI as the label while inserting only the untyped suffix', async () => {
            ...
            const tokens = [makeToken(RdfToken.IRIREF.name, '<workspace')] as any;
            const items = await p.getGraphIriCompletionItems(makeDoc('file:///test.sparql', '<workspace') as any, tokens, 0);
            expect(items.length).toBe(1);
        ...
        it('includes workspace graphs provided by the connection', async () => {
            const p = makeProviderWithGraphs(['workspace:///data.ttl']);
            const tokens = [makeToken(RdfToken.IRIREF.name, '<')] as any;
            const items = await p.getGraphIriCompletionItems(makeDoc('file:///w/query.sparql') as any, tokens, 0);
            const labels = items.map(i => i.label as string);
        ...
        it('includes notebook cell workspace URIs in completion candidates', async () => {
            ...
            const tokens = [makeToken(RdfToken.IRIREF.name, '<')] as any;
            const items = await p.getGraphIriCompletionItems(makeDoc('vscode-notebook-cell:///w/notebook.mnb#opaqueCell2') as any, tokens, 0);
            const labels = items.map(i => i.label as string);
        ...
        it('matches workspace cell IRIs by slug substring when prefix does not match', async () => {
            ...
            // find it.
            const p = makeProviderWithGraphs(['workspace:///notebook.mnb#my-data']);
            const tokens = [makeToken(RdfToken.IRIREF.name, '<workspace:///my-data')] as any;
            const items = await p.getGraphIriCompletionItems(
        ...
        it('does not apply substring matching to non-workspace IRIs', async () => {
            ...
            const tokens = [makeToken(RdfToken.IRIREF.name, '<workspace:///cell')] as any;
            const items = await p.getGraphIriCompletionItems(
        ...
        it('uses prefix match (not substring) when the typed value already prefixes the IRI', async () => {
            ...
            const tokens = [makeToken(RdfToken.IRIREF.name, '<workspace:///notebook')] as any;
            const items = await p.getGraphIriCompletionItems(
    ...
    describe('getCompletionItems', () => {
        it('delegates to getGraphIriCompletionItems when in graph definition context', async () => {
            ...
            await p.getCompletionItems(doc, context, tokens, 1);
            expect(p.getGraphIriCompletionItems).toHaveBeenCalledWith(doc, tokens, 1);
        });
        ...
        it('calls super.getCompletionItems when not in graph definition context', () => {
            ...
            const result = p.getCompletionItems(doc, context, tokens, 1);
    ...
    describe('provideCompletionItems', () => {
        ...
        it('returns null when the tokens do not cover the position', async () => {
            ...
            const result = await p.provideCompletionItems(
                makeDoc() as any, { line: 0, character: 0 } as any, {} as any, {} as any
            );
            expect(result).toBeNull();
        });
        ...
        it('tokenizes the document text and calls getCompletionItems with the fresh tokens', async () => {
            ...
            const doc = makeDoc('file:///test.sparql', '. ex:') as any;
            // Position inside the 'ex:' token (0-based line 0, character 3 → 1-based col 3..5).
            await p.provideCompletionItems(
                doc, { line: 0, character: 3 } as any, {} as any, {} as any
            );
            expect(tokenize).toHaveBeenCalledWith('. ex:');
