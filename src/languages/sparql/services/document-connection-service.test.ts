...
function _buildCellUri(notebookPath: string) {
    const base = vscode.Uri.parse(notebookPath);
...
describe('DocumentConnectionService', () => {
    describe('getConnectionForDocument', () => {
        it('returns the workspace store when no connection is set for the document', () => {
            ...
            const conn = svc.getConnectionForDocument(uri);
    ...
    describe('notifyDocumentConnectionChanged', () => {
        it('fires the onDidChangeConnectionForDocument event with the URI', () => {
            ...
            svc.notifyDocumentConnectionChanged(uri);
    ...
    describe('getInferenceEnabledForDocument', () => {
        it('returns false for a plain file URI with no stored setting', () => {
            const svc = makeService();
            const uri = vscode.Uri.parse('file:///test.sparql');
            expect(svc.getInferenceEnabledForDocument(uri)).toBe(false);
        });

        it('returns the stored document-level setting when one is set', async () => {
            const svc = makeService();
            const uri = vscode.Uri.parse('file:///doc.sparql');
            await svc.setInferenceEnabledForDocument(uri, true);
            expect(svc.getInferenceEnabledForDocument(uri)).toBe(true);
        });
    });
    ...
    describe('toggleInferenceEnabledForDocument', () => {
        it('toggles false to true for a file URI', async () => {
            const svc = makeService();
            const uri = vscode.Uri.parse('file:///doc2.sparql');
            // Default is false
            const result = await svc.toggleInferenceEnabledForDocument(uri);
    ...
    describe('setQuerySourceForDocument', () => {
        ...
        it('invokes setConnectionForCell for a notebook-cell URI', async () => {
            ...
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb-sq.sparql-book'), getCells: () => [mockCell] };
    ...
    describe('getConnectionForDocument — notebook-cell branch', () => {
        it('returns workspace store when cell has no connectionId metadata', async () => {
            ...
            const mockCell = { document: { uri: cellUri }, metadata: {}, index: 0 };
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb.sparql-book'), getCells: () => [mockCell] };
            (vscode.workspace as any).notebookDocuments = [mockNotebook];
    ...
    describe('getInferenceEnabledForDocument — notebook-cell branch', () => {
        it('returns undefined (falls back to connection) when cell has no inferenceEnabled metadata', async () => {
            ...
            const mockCell = { document: { uri: cellUri }, metadata: {}, index: 0 };
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb2.sparql-book'), getCells: () => [mockCell] };
            (vscode.workspace as any).notebookDocuments = [mockNotebook];
        ...
        it('returns the cell-level inferenceEnabled when set in metadata', async () => {
            ...
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb3.sparql-book'), getCells: () => [mockCell] };
            (vscode.workspace as any).notebookDocuments = [mockNotebook];
    ...
    describe('setConnectionForCell', () => {
        ...
        it('throws when cell is not found in the notebook', async () => {
            ...
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb4.sparql-book'), getCells: () => [] };
            (vscode.workspace as any).notebookDocuments = [mockNotebook];
        ...
        it('applies workspace edit when cell is found', async () => {
            ...
            const mockCell = { document: { uri: cellUri }, metadata: {}, index: 0 };
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb5.sparql-book'), getCells: () => [mockCell] };
            (vscode.workspace as any).notebookDocuments = [mockNotebook];
    ...
    describe('setInferenceEnabledForDocument — notebook-cell branch', () => {
        ...
        it('throws when cell is not found in notebook', async () => {
            ...
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb6.sparql-book'), getCells: () => [] };
            (vscode.workspace as any).notebookDocuments = [mockNotebook];
        ...
        it('applies a workspace edit to set inferenceEnabled on the cell', async () => {
            ...
            const mockCell = { document: { uri: cellUri }, metadata: {}, index: 0 };
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb7.sparql-book'), getCells: () => [mockCell] };
            (vscode.workspace as any).notebookDocuments = [mockNotebook];
        ...
        it('applies a workspace edit to clear inferenceEnabled (undefined)', async () => {
            ...
            const mockCell = { document: { uri: cellUri }, metadata: { inferenceEnabled: true }, index: 0 };
            const mockNotebook = { uri: vscode.Uri.parse('file:///nb8.sparql-book'), getCells: () => [mockCell] };
            (vscode.workspace as any).notebookDocuments = [mockNotebook];
    ...
    describe('_onNotebookDocumentChanged — adds cells with inherited settings', () => {
        it('inherits connectionId and inferenceEnabled from the previous cell when new cells are added', async () => {
            ...
            const nbUri = vscode.Uri.parse('file:///nb9.sparql-book');
        ...
        it('does not inherit settings when new cell already has metadata', async () => {
            ...
            const nbUri = vscode.Uri.parse('file:///nb10.sparql-book');
    ...
    describe('handleFileRenames', () => {
        it('migrates sparql.connection: key on file rename', async () => {
            ...
            await svc.handleFileRenames([{
                oldUri: vscode.Uri.parse('file:///workspace/old.ttl'),
                newUri: vscode.Uri.parse('file:///workspace/new.ttl'),
            }]);
        ...
        it('migrates all keys under a renamed folder', async () => {
            ...
            await svc.handleFileRenames([{
                oldUri: vscode.Uri.parse('file:///workspace/models'),
                newUri: vscode.Uri.parse('file:///workspace/renamed'),
            }]);
    ...
    describe('untitled SPARQL save migration', () => {
        function makeServiceWithCapturedSaveHandlers(initial: Record<string, any> = {}) {
            ...
            (vscode.workspace as any).onDidOpenTextDocument = (handler: any) => {
                openHandler = handler;
                return { dispose: () => {} };
            };
            ...
            (vscode.workspace as any).onDidOpenTextDocument = (_handler: any) => ({ dispose: () => {} });
            (vscode.workspace as any).onDidChangeTextDocument = (_handler: any) => ({ dispose: () => {} });
            (vscode.workspace as any).onDidSaveTextDocument = (_handler: any) => ({ dispose: () => {} });
        ...
        it('migrates the connection key from the untitled URI to the saved file URI', async () => {
            ...
            openHandler({ isUntitled: true, languageId: 'sparql', uri: vscode.Uri.parse('untitled:Untitled-1'), getText: () => content });
            await didSaveHandler({ isUntitled: false, languageId: 'sparql', uri: vscode.Uri.parse('file:///workspace/new.sparql'), getText: () => content });
        ...
        it('migrates using the latest content captured from a text change', async () => {
            ...
            await didSaveHandler({ isUntitled: false, languageId: 'sparql', uri: vscode.Uri.parse('file:///workspace/new.sparql'), getText: () => 'SELECT * WHERE { ?s ?p ?o }' });
        ...
        it('does not migrate when the saved content does not match a tracked untitled document', async () => {
            ...
            openHandler({ isUntitled: true, languageId: 'sparql', uri: vscode.Uri.parse('untitled:Untitled-1'), getText: () => 'SELECT * WHERE { ?s ?p ?o }' });
            await didSaveHandler({ isUntitled: false, languageId: 'sparql', uri: vscode.Uri.parse('file:///workspace/new.sparql'), getText: () => 'a different query' });
        ...
        it('ignores saves of non-SPARQL or already-titled documents', async () => {
            ...
            await didSaveHandler({ isUntitled: false, languageId: 'turtle', uri: vscode.Uri.parse('file:///workspace/new.ttl'), getText: () => 'SELECT * WHERE { ?s ?p ?o }' });
            await didSaveHandler({ isUntitled: true, languageId: 'sparql', uri: vscode.Uri.parse('untitled:Untitled-1'), getText: () => 'SELECT * WHERE { ?s ?p ?o }' });
